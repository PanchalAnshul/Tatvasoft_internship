...
export class UpdateUserComponent implements OnInit, OnDestroy {
  ...
  onSubmit() {
    console.log("submit" + this.updateForm.valid);
    
    if (this.updateForm.invalid) {
      const formData = new FormData();
      const updatedUserData = this.updateForm.getRawValue(); // includes disabled fields
          ...
          if (data.result == 1) {
            ...
            setTimeout(() => {
              this._router.navigate(
                this.isupdateProfile ? ['admin/profile'] : ['admin/user']
              );
            }, 1000);
          } else {
